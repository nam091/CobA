...
### 1.3. OpenAI / Anthropic-compatible endpoints (BYO gateway)

Trong M2 hệ thống được mở rộng để hỗ trợ **endpoint tùy chỉnh**: người dùng tự
quyết LLM nào sẽ chạy detector/verifier (cloud, gateway tổng hợp, hay self-hosted)
chỉ bằng 2 cặp biến môi trường:

| Biến | Tác dụng | Mặc định |
|---|---|---|
| `OPENAI_BASE_URL` | Trỏ `AsyncOpenAI` sang gateway tương thích OpenAI | `None` → dùng `api.openai.com` |
| `OPENAI_API_KEY` | Key của gateway (vài endpoint private không cần) | — |
| `ANTHROPIC_BASE_URL` | Trỏ `AsyncAnthropic` sang relay tương thích Anthropic | `None` → dùng `api.anthropic.com` |
| `ANTHROPIC_API_KEY` | Key của relay Anthropic | — |
| `COBA_LLM_DETECTOR` | Model id detector | `gpt-4o-mini` |
| `COBA_LLM_VERIFIER` | Model id verifier | `claude-3-5-sonnet-20241022` |

Gateway phổ biến đã test:

| Gateway | `OPENAI_BASE_URL` | Mẫu model id |
|---|---|---|
| OpenRouter | `https://openrouter.ai/api/v1` | `qwen/qwen-2.5-coder-32b-instruct` |
| Together AI | `https://api.together.xyz/v1` | `meta-llama/Llama-3.1-8B-Instruct` |
| Groq | `https://api.groq.com/openai/v1` | `llama-3.1-70b-versatile` |
| DeepSeek | `https://api.deepseek.com/v1` | `deepseek-coder` |
| Mistral | `https://api.mistral.ai/v1` | `codestral-latest` |
| Fireworks | `https://api.fireworks.ai/inference/v1` | `accounts/fireworks/models/...` |
| Ollama /v1 shim | `http://localhost:11434/v1` | `qwen2.5-coder:7b` |
| vLLM / LiteLLM / TGI | tự host | tùy hệ |

**Cơ chế**: khi `OPENAI_BASE_URL` được set, `OpenAIProvider.supports()` chấp
nhận mọi model id (trừ namespace `claude-*` và `gemini-*`), và
`OpenAIProvider._get_client()` truyền `base_url` thẳng cho `AsyncOpenAI`. Khi
key rỗng nhưng base URL có giá trị (endpoint private), provider tự gắn
`api_key="sk-noauth"` để SDK không complain. Xem chi tiết:
[`src/coba/llm/openai_provider.py`](../src/coba/llm/openai_provider.py).

**Lưu ý chi phí**: với gateway lạ, bảng `coba.llm.cost.price_for` thường không
có entry → cost mặc định = 0. Người dùng nên override
`COBA_LLM_DAILY_BUDGET_USD` về 0 (tắt budget guard) hoặc nạp `price_for`
custom nếu muốn tracking đúng.
    ...
    def __init__(self, settings: Settings):
        self.providers: dict[str, LLMProvider] = {
            "openai":   OpenAIProvider(
                ...
                base_url=settings.openai_base_url,   # custom OpenAI-compatible endpoint
            ),
            "anthropic": AnthropicProvider(
                settings.anthropic_api_key,
                base_url=settings.anthropic_base_url,  # custom Anthropic relay
            ),
